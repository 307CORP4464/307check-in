...
export default function AppointmentsPage() {
  ...
  const workInCount = searchFilteredAppointments.filter(apt => apt.appointment_time?.toLowerCase().includes('work in')).length;
  const checkedOutCount = searchFilteredAppointments.filter(apt => {
    const s = getDailyLogStatus(apt)?.toLowerCase();
    return s === 'checked_out' || s === 'unloaded';
  }).length;
  const checkedInCount = searchFilteredAppointments.filter(apt => getDailyLogStatus(apt)?.toLowerCase() === 'checked_in').length;
  const pendingCount = searchFilteredAppointments.filter(apt => getDailyLogStatus(apt)?.toLowerCase() === 'pending').length;
  const notCheckedInCount = searchFilteredAppointments.filter(apt => !getDailyLogStatus(apt)).length;

  // ── Enhanced duplicate check: searches ALL appointments, not just selected date ──
  const handleCheckDuplicate = async (salesOrder: string, delivery: string) => {
    ...
    try {
      // Build OR filter to search across all appointments
      const orParts: string[] = [];
      if (so) {
        orParts.push(`sales_order.ilike.%${so}%`);
      }
      if (del) {
        orParts.push(`delivery.ilike.%${del}%`);
      }
      ...
      setExistingAppointment(data && data.length > 0 ? (data[0] as Appointment) : null);
    } catch (err) {
      console.error('Error in handleCheckDuplicate:', err);
    }
  };
...
const fetchCheckInStatuses = async () => {
  ...
  const statusBreakdownButtons: {
    ...
    { filter: 'all', label: 'Total', count: totalAppointmentsCount, activeClass: 'bg-blue-600 text-white border-blue-600', inactiveClass: 'bg-white text-blue-700 border-blue-300 hover:bg-blue-50' },
    { filter: 'not_checked_in', label: 'Not Checked In', count: notCheckedInCount, activeClass: 'bg-blue-500 text-white border-blue-500', inactiveClass: 'bg-white text-blue-600 border-blue-200 hover:bg-blue-50' },
    { filter: 'checked_in', label: 'Checked In', count: checkedInCount, activeClass: 'bg-purple-600 text-white border-purple-600', inactiveClass: 'bg-white text-purple-700 border-purple-300 hover:bg-purple-50' },
    { filter: 'pending', label: 'Pending', count: pendingCount, activeClass: 'bg-yellow-500 text-white border-yellow-500', inactiveClass: 'bg-white text-yellow-700 border-yellow-300 hover:bg-yellow-50' },
    { filter: 'checked_out', label: 'Checked Out', count: checkedOutCount, activeClass: 'bg-green-600 text-white border-green-600', inactiveClass: 'bg-white text-green-700 border-green-300 hover:bg-green-50' },
    { filter: 'work_in', label: 'Work In', count: workInCount, activeClass: 'bg-gray-700 text-white border-gray-700', inactiveClass: 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' },
  ];
      ...
      <div className="max-w-[1600px] mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white border-2 border-gray-200 rounded-lg shadow-lg p-4">
            <h3 className="text-sm font-bold text-gray-700 mb-3 text-center">Appointments by Customer</h3>
          ...
          <div className="bg-white border-2 border-gray-200 rounded-lg shadow-lg p-4">
            <h3 className="text-sm font-bold text-gray-700 mb-3 text-center">Search Appointments</h3>
            <div className="relative mb-4">
              ...
              {searchQuery && (
                <button onClick={clearSearch} className="absolute right-3 top-3 text-gray-400 hover:text-gray-600">✕</button>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {statusBreakdownButtons.map(({ filter, label, count, activeClass, inactiveClass }) => (
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition-colors duration-150 cursor-pointer ${isFilterActive(filter) ? activeClass : inactiveClass}`}
                >
                  {label}
                  <span className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-xs font-bold ${isFilterActive(filter) ? 'bg-white bg-opacity-25 text-inherit' : 'bg-gray-100 text-gray-600'}`}>
                    {count}
                  </span>
                </button>
              ))}
            ...
            {statusFilter !== 'all' && (
              <p className="mt-2 text-xs text-gray-500">
                Showing <span className="font-semibold">{filteredAppointments.length}</span> appointment{filteredAppointments.length !== 1 ? 's' : ''} with status:{' '}
                <span className="font-semibold capitalize">{statusBreakdownButtons.find(b => b.filter === statusFilter)?.label}</span>
              </p>
            )}
          </div>
        </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sales Order</th>
                        ...
                        <td className="px-4 py-3 text-sm text-gray-700">
                          <div className="flex flex-col">
                            {appointment.requested_ship_date && (
                              <span className="font-medium text-gray-900 text-xs">{appointment.requested_ship_date}</span>
                            )}
                            {(appointment.ship_to_city || appointment.ship_to_state) && (
                              <span className="text-xs text-gray-500">
                                {[appointment.ship_to_city, appointment.ship_to_state].filter(Boolean).join(', ')}
                              </span>
                            )}
                            {!appointment.requested_ship_date && !appointment.ship_to_city && !appointment.ship_to_state && <span>-</span>}
                          </div>
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-700">
                          <div className="flex flex-col">
                            {appointment.carrier && <span className="font-medium text-gray-900">{appointment.carrier}</span>}
                            {appointment.mode && <span className="text-xs text-gray-500">{appointment.mode}</span>}
                            {!appointment.carrier && !appointment.mode && <span>-</span>}
                          </div>
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-900">{appointment.customer}</td>
                        <td className="px-4 py-3 text-sm">{appointment.notes || '-'}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm">{getStatusBadge(dailyLogStatus)}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm space-x-2">
                          <button onClick={() => handleEdit(appointment)} className="text-blue-600 hover:text-blue-800 font-medium">Edit</button>
                          <button onClick={() => handleDelete(appointment.id)} className="text-red-600 hover:text-red-800 font-medium">Delete</button>
                        </td>
                      </tr>
                    );
                  })}
